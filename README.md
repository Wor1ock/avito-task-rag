...
## Что происходит при запуске:
   ...
   4. происходит пакетный расчет ответов для test.f. для каждого запроса каждый ретривер (BM25 и FAISS) извлекает по 100 кандидатов (`top_k_candidates`), списки объединяются через RRF, и топ-15 объединённого пула переранжируется `BAAI/bge-reranker-v2-m3`. итоговый топ-10 сохраняется в файл answer.csv в корне проекта.
