...
| :--- | :---: | :--- |
| **Baseline** | **0,4543** | Гибридный поиск, реранкинг, чанкинг по 1500 токенов |
...
## Что происходит при запуске:
   ...
   2. если файлы индексов отсутствуют в data/, автоматически строятся два индекса: лексический (`BM25Okapi` по полным статьям) и векторный семантический — Markdown-текст каждой статьи режется на чанки по 500 символов с перекрытием 50 (`model.chunk_size` / `model.chunk_overlap`), каждый чанк кодируется моделью `paraphrase-multilingual-MiniLM-L12-v2` и хранится в FAISS вместе с `article_id` родительской статьи;
   3. скрипт делает предсказания для calibration.f, вычисляет точную метрику MAP@10 и выводит её значение в консоль;
   4. происходит пакетный расчет ответов для test.f. для каждого запроса BM25 извлекает 100 статей-кандидатов (`top_k_candidates`), FAISS ищет по чанкам (сырой текст запроса, без нормализации), и найденные чанки схлопываются до родительских статей; списки объединяются через RRF, и топ-15 объединённого пула (`reranker.rerank_depth`) переранжируется `BAAI/bge-reranker-v2-m3` по лучшему чанку каждой статьи. итоговый топ-10 `article_id` сохраняется в файл answer.csv в корне проекта.
